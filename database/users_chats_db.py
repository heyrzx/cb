...
class Database:
    ...
    async def get_autofilter(self):
        data = await self.admn.find_one({'id': 'auto'})
        if not data:
            data = {
                'id': 'auto',
                'autofilter': True
            }
            await self.admn.insert_one(data)
        return data

    async def update_autofilter(self, value):
        await self.adm.update_one({'id': 'auto'}, {'$set': {'autofilter': value}})
